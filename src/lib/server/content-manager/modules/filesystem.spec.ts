...
describe('Content-Manager: Filesystem', () => {
  ...
  describe('Filesystem', () => {
    ...
    it('write file', async () => {
      ...
      expect((await fs.readFile(join(TEST_BASE_PATH, 'city_image.jpg'))).toString()).toBe(image.toString())
    })
  ...
  describe('JsonFilesystem', () => {
    ...
    it('write object', async () => {
      ...
      expect(await fs.readFile(join(TEST_BASE_PATH, 'obj.json'), 'utf-8')).toEqual(JSON.stringify(obj));
    })
    ...
    it('get partial data but save all', async () => {
      ...
      const content = JSON.parse((await fs.readFile(join(TEST_BASE_PATH, `${filename}.json`))).toString());
      ...
      const changedContent = JSON.parse((await fs.readFile(join(TEST_BASE_PATH, `${filename}.json`))).toString());
