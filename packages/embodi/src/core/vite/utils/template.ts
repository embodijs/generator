...
export const loadLayouts = async (cwd: string = process.cwd(), { inputDirs: { layout } }: EmbodiConfig): Promise<Record<string, Layout> | undefined> => {
...
export const importLayout = async (module: string): Promise<Layout> => (await import(module)).default;
